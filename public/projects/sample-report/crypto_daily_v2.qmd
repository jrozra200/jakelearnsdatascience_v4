...
plot_dat <- py$pred_dat %>% 
    ...
    left_join(spy %>% 
                  select(Date, Close), 
              by = c("pred_date" = "Date")) %>% 
    mutate(dotw = wday(pred_date, label = TRUE),
           Close = ifelse(is.na(Close) & dotw == "Fri", lag(Close), Close),
           Close = ifelse(is.na(Close) & dotw == "Thu", lag(Close), Close),
           Close = ifelse(is.na(Close) & dotw == "Wed", lag(Close), Close),
           Close = ifelse(is.na(Close) & dotw == "Tue", lag(Close), Close),
           Close = ifelse(is.na(Close) & dotw == "Mon", lag(Close, 3), Close),
           Close = case_when(dotw == "Sat" ~ lag(Close),
                             dotw == "Sun" ~ lag(Close, 2),
                             1 == 1 ~ Close),
           spy_buy = total_day_invested / Close,
           cum_spy_buy = cumsum(spy_buy),
           spy_sell = total_day_returned / Close,
           cum_spy_sell = cumsum(spy_sell),
           spy_port = (cum_spy_buy * Close) + cum_spy_sell,
           spy_up_down = (spy_port - total_invested) / total_invested) %>% 
    select(-dotw) %>% 
    pivot_longer(!pred_date, names_to = "metric", values_to = "value") %>% 
    filter((metric == "up_down" | metric == "spy_up_down") &
               value >= -0.25) %>% 
    mutate(metric = case_when(metric == "up_down" ~ "RLE Analytics Returns",
...
cryp_dat <- py$pred_dat %>% 
    ...
    mutate(pred_date = as_date(pred_date),
           ...
           decision = case_when(
               ...
               buy ~ "buy",
               1 == 1 ~ "hold"),
           dotw = wday(pred_date, label = TRUE),
    ...
    filter(pred_date != "2024-05-28") %>% 
    ungroup() %>% 
    group_by(pred_date, quarter, year, dotw) %>% 
    summarise(total_day_invested = sum(norm_day_invested),
              total_day_returned = sum(norm_day_returned),
              total_invested = sum(norm_invested),
              total_returned = sum(norm_returned),
              total_port_value = sum(norm_port_value),
              up_down = (total_port_value - total_invested) / total_invested) %>% 
    rename("RLE Analytics Returns" = "up_down")
...
spy_invest <- spy %>% 
    ...
    left_join(cryp_dat %>% 
                  ...
                  summarise(total_invested = sum(total_day_invested)),
              by = c("quarter" = "quarter",
                     "year" = "year")) %>% 
    group_by(quarter, year) %>% 
    summarise(first_date = min(Date),
              total_invested = max(total_invested),
              purchased = ifelse(first_date == Date, total_invested / Close, NA)) %>% 
    filter(!is.na(purchased))

spy_dat <- spy %>% 
    mutate(quarter = quarter(Date),
           year = year(Date)) %>% 
    left_join(spy_invest, by = c("quarter", "year")) %>% 
    mutate(spy_value = purchased * Close,
           spy_up_down = (spy_value - total_invested) / total_invested)

plot_dat <- cryp_dat %>% 
    left_join(spy_dat, by = c("pred_date" = "Date")) %>% 
