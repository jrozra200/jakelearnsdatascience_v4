...
model_performance <- function(today, dat, token, buy_thresh, sell_thresh) {
    ...
    ggplot(plot_dat, aes(x = pred_date)) +
        geom_errorbar(aes(ymin = price, ymax = price_30d, color = confusion), width = 0) +
        geom_line(aes(y = price)) + 
        geom_point(aes(color = confusion, shape = decision), size = 2, 
                   y = min(plot_dat$price)) + 
        scale_color_manual(values = c("correct" = "forestgreen", 
                                      "incorrect" = "red", 
                                      "unknown" = "blue")) + 
        scale_shape_manual(values = c("B" = "B", "H" = "H", "S" = "S")) + 
        scale_y_continuous(labels = dollar_format()) + 
        ggtitle("Price of BTC & Buy Model Decision",
...
BTC is going down by 10% in the next 30 days.

ETH is going down by 10% in the next 30 days.
...
model_performance(py$btc_today, btc, "BTC", 0.13, 0.6)
```

### ETH

```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.height = 4, out.width = "100%"}
model_performance(py$eth_today, eth, "ETH", 0.17, 0.6)
```
